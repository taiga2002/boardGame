...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i++) {
            for (int k = 0; k < piece.width(); k++) {
                if (piece.get(i, k) && get(row + i, col + k)) {
                    return false;
                }
            }
        }
    ...
    int[][] rowColumnCounts() {
        ...
        for (int i = 0; i < height(); i++) {
            for (int k = 0; k < width(); k++) {
                result[0][i] += get(i, k) ? 1 : 0;
                result[1][k] += get(i, k) ? 1 : 0;
            }
        }
        return result;
    }
    ...
    void clearFilledLines() {
        ...
        _streakLength = (nrows == 0 && ncols == 0) ? 0 : _streakLength + 1;
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int basic = nrows * width() + ncols * height() - nrows * ncols;
        int bonus = _streakLength * (nrows * width() + ncols * height());
        return basic + bonus;
    }
